...
def resolve_device(name):
    """
    Turn the device from the config into a real one.

    'auto' picks the accelerator that exists: CUDA, then Apple MPS, then the
    CPU. Anything else is passed through untouched.

    Args:
        name (str): device from the config, or 'auto'.
    Returns:
